...
const PreviousResults = () => {
  ...
  useEffect(() => {
    const fetchResumes = async () => {
      try {
        const res = await axiosInstance.post('http://localhost:8000/api/v1/resume/history');
        setResumes(res.data.resumes || []);
      } catch (error) {
        toast.error(error?.response?.data?.detail[0]?.msg || error.response?.data?.detail || 'Failed to fetch past results');
      }
    };
      ...
      {resumes.length === 0 ? (
        ...
        resumes.map((resume, idx) => (
          ...
          >
            ...
            <p><strong>Uploaded:</strong> {new Date((resume.uploadedAt)*1000).toLocaleString()}</p>
