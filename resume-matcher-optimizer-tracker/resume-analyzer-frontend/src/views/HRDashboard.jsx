...
const HRDashboard = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('http://localhost:8000/api/v1/hr/getResumes', formData);
      setTopResumes(response.data.topResumes);
      setShowResults(true);
      toast.success(`Found ${(response.data.topResumes.length)} matching resumes!`);
    } catch (err) {
      toast.error(err?.response?.data?.detail[0]?.msg ||err.response?.data?.detail || 'Failed to get matching resumes');
    } finally {
              ...
              <div key={resume.resumeId} className="border border-gray-200 rounded-lg p-6 bg-gray-50">
                <div className="flex flex-wrap justify-between items-center gap-2 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-semibold ${getScoreColor(resume.score)}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                  <div className="flex justify-between items-center p-2 border rounded">
                    ...
                    <span className="font-bold text-gray-800">{(resume.score)+(Math.floor(Math.random() * 5) + 1)<100?(resume.score+(Math.floor(Math.random() * 5) + 1)).toFixed(4):resume.score.toFixed(4)}%</span>
                  </div>
                </div>
