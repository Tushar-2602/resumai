...
const UploadView = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post('http://localhost:8000/api/v1/resume/saveResumeOrScore', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      toast.success('Analysis complete!');
      navigate('/results', { state: res.data });
    } catch (error) {
      toast.error(error?.response?.data?.detail[0]?.msg ||error?.response?.data?.detail || 'Upload failed');
    } finally {
